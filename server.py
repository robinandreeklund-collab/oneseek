...
if __name__ == "__main__":
    ...
    try:
        ...
        # Set the appropriate logging level for the backend.deer_flow package if debug is enabled
        if log_level.lower() == "debug":
            logging.getLogger("backend.deer_flow").setLevel(logging.DEBUG)
            logging.getLogger("langchain").setLevel(logging.DEBUG)
            logging.getLogger("langgraph").setLevel(logging.DEBUG)
            logger.info("DEBUG logging enabled for backend.deer_flow, langchain, and langgraph packages - detailed diagnostic information will be logged")
        
        uvicorn.run(
            "backend.deer_flow.server:app",
            host=args.host,
            port=args.port,
            reload=reload,
            log_level=log_level,
